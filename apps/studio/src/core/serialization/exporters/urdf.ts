...
/** origin/transform-like { position, quaternion } → Matrix4 (scale ignored). */
function mat4(o: any) {
  return new THREE.Matrix4().compose(
    new THREE.Vector3(...(o?.position ?? [0, 0, 0])),
    new THREE.Quaternion(...(o?.quaternion ?? [0, 0, 0, 1])),
...
function jointXML(j: any, doc: any) {
  ...
  // URDF joint frame = child link at joint value 0, relative to the parent. Our FK
  // gives child(0) = parent ∘ origin ∘ childRest, so the URDF <origin> must be the
  // COMPOSED origin ∘ childRest — not just `origin` (which ignored childRest and
  // misplaced snapped / pivot-edited joints).
  const Ojoint = mat4(j.origin).multiply(mat4(j.childRest));
  const jp = new THREE.Vector3().setFromMatrixPosition(Ojoint);
  const jq = new THREE.Quaternion().setFromRotationMatrix(Ojoint);

  // Our joint axis is expressed in the pivot (origin) frame; URDF expects it in the
  // child/joint frame → rotate it by childRest⁻¹.
  const crq = new THREE.Quaternion().setFromRotationMatrix(mat4(j.childRest)).invert();
  const ax = new THREE.Vector3(...(j.axis ?? [0, 0, 1])).applyQuaternion(crq);
  if (ax.lengthSq() > 1e-9) ax.normalize();
  ...
  return `  <joint name="${san(j.name)}" type="${j.type}">
    ...
    <origin xyz="${xyz([jp.x, jp.y, jp.z])}" rpy="${rpy([jq.x, jq.y, jq.z, jq.w])}"/>
    <axis xyz="${xyz([ax.x, ax.y, ax.z])}"/>${limXML}
...
export function exportURDF(doc: Document, name = 'tetrobot') {
  ...
  // URDF needs exactly ONE root link. When our graph has several roots (or none,
  // i.e. all-cycle), synthesize a `base_link` and fix every root under it at its
  // world transform, so the export is always a single valid tree.
  const needsBase = roots.length !== 1;
  const baseTargets = roots.length ? roots : bodies.slice(0, 1);
  const baseLinkXML = needsBase ? '  <link name="base_link"/>\n' : '';
  const baseJointsXML = needsBase
    ? baseTargets.map((r) => {
        const t = r.transform ?? {};
        return `  <joint name="base_to_${san(r.name)}" type="fixed">
    <parent link="base_link"/>
    <child link="${san(r.name)}"/>
    <origin xyz="${xyz(t.position ?? [0, 0, 0])}" rpy="${rpy(t.quaternion ?? [0, 0, 0, 1])}"/>
  </joint>`;
      }).join('\n')
    : '';

  const warns: string[] = [];
  if (needsBase) warns.push(`Added a synthetic base_link joining ${baseTargets.length} root link(s) with fixed joints (URDF requires one tree).`);
  if (loopJoints.length) warns.push(`${loopJoints.length} loop/cross joint(s) can't be represented in a URDF tree and were dropped: ${loopJoints.map((j) => san(j.name)).join(', ')}.`);
...
<robot name="${san(name)}">
${header}${baseLinkXML}${links}
${baseJointsXML ? baseJointsXML + '\n' : ''}${joints}
</robot>
`;
}