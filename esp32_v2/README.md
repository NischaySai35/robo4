...
Open `esp32_v2/esp32_v2.ino`, set `DEF_SSID` / `DEF_PASS` if your network changed, upload
...
`arduino-cli upload -p mod1.local --fqbn esp32:esp32:esp32c3 esp32_v2`.

### b) Drag a .bin into the web page
**Sketch → Export Compiled Binary**, then open `http://mod1.local/` → **OTA update** tab →
pick `esp32_v2.ino.bin` → Upload. Progress bar, auto-reboot, page reloads itself.

Same thing from a terminal:
```
curl -F "f=@esp32_v2.ino.bin" http://mod1.local/api/ota
```
...
.\tools\flash-all.ps1 -NoBuild -Bin .\esp32_v2.ino.bin
```
