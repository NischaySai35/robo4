/*
   ROBO4 — per-MODULE firmware: ESP32-C3 + up to 7 × ST3215 servos + 2 electromagnets
   JSON API only (no embedded HTML).

   MULTI-ESP: every module runs this same sketch; set MDNS_HOST (mod1/mod2/…) per board.
   The laptop app is the master and polls all boards continuously. Servo + magnet ids
   are LOCAL to each board (servos 1..7, magnet channels 0..1).

   React web app talks to each board via:
     GET /api/telemetry          → full JSON for this board's servos + magnets
     GET /api/command?servo=N&cmd=X[&angle=Y&speed=Z&acc=W]
     GET /api/batch?1=180&2=90&3=150&4=180&5=120&6=180&7=180[&speed=5&acc=20]
     GET /api/magnet?ch=0&pct=100    → set an end-lock electromagnet's power %
...
// ── PER-MODULE IDENTITY ─────────────────────────────────────────────────────────
// MULTI-ESP: every module runs THIS SAME sketch on its own ESP32-C3. The laptop
// (master) talks to all boards at once as http://<MDNS_HOST>.local. The ONLY thing
// to change per board before uploading is this name: mod1, mod2, mod3, …
// Then bind each board to its module in the app's Motor Control → ESP32 Nodes list.
constexpr const char* MDNS_HOST = "mod1";

// ── Hardware ──────────────────────────────────────────────────────────────────
#define SERVO_PIN   5
#define SERVO_BAUD  1000000

// ── DRV8833 electromagnet driver (2 channels = this module's 2 end-lock magnets) ─
//   Channel A: IN1=GPIO6  IN2=GPIO7      Channel B: IN1=GPIO8  IN2=GPIO9
// Forward drive (energize) = IN1 PWM, IN2 low. Coast (release) = both low.
// Magnets have NO feedback — the master (laptop) computes the 100%-grab→40%-hold
// profile and streams the target %; we just apply it. A firmware safety net eases a
// magnet down to a safe hold if the master goes silent while it's still at full grab.
#define MAG_COUNT       2
const uint8_t MAG_IN1[MAG_COUNT] = { 6, 8 };
const uint8_t MAG_IN2[MAG_COUNT] = { 7, 9 };
constexpr uint32_t MAG_PWM_FREQ  = 20000; // 20 kHz — inaudible
constexpr uint8_t  MAG_PWM_RES   = 8;     // 0..255 duty
constexpr uint8_t  MAG_SAFE_HOLD = 40;    // % to fall back to if master goes quiet at full grab
constexpr uint32_t MAG_GRAB_MAX_MS = 4000; // max time a magnet may sit above safe-hold w/o a new cmd

struct MagnetState {
  uint8_t  pct;            // last commanded power %
  unsigned long lastCmdMs; // when it was last commanded (for the safety taper)
};
MagnetState magnets[MAG_COUNT] = { {0, 0}, {0, 0} };

// Apply a power % to one magnet channel via the DRV8833.
void setMagnet(uint8_t ch, int pct) {
  ...
  ledcWrite(MAG_IN1[ch], duty); // ESP32 core 3.x: write by pin
...
// GET /api/magnet?ch=0&pct=100   → set one electromagnet channel's power %.
// GET /api/magnet?all=1&pct=0     → set every channel (e.g. release-all).
void handleMagnet() {
  setCORS();
  ...
  if (ch < 0 || ch >= MAG_COUNT) {
    server.send(404, "application/json", "{\"ok\":false,\"error\":\"bad channel\"}");
    return;
  }
...
void setup() {
  ...
  // DRV8833 magnet PWM (ESP32 core 3.x LEDC-by-pin). Start released (0%).
  for (uint8_t i = 0; i < MAG_COUNT; i++) {
...
void loop() {
  ...
  // Magnet safety taper: if the master stops streaming while a magnet is still above
  // the safe hold %, ease it down to MAG_SAFE_HOLD so a lost link can't leave a coil
  // pinned at full grab (heat). A normal lock stays latched at hold; releases are 0%.
  for (uint8_t i = 0; i < MAG_COUNT; i++) {
    if (magnets[i].pct > MAG_SAFE_HOLD && (millis() - magnets[i].lastCmdMs) > MAG_GRAB_MAX_MS) {
      setMagnet(i, MAG_SAFE_HOLD);
    }
  }
