...
export function makeSystemPrompt(doc) {
  return [
...
function entityByName(doc, kind, name) {
...
export function sanitizePlan(plan, doc) {
  const safe = [];
...
export function localPlan(input, doc) {
...
export function executeAiPlan(plan, { doc, dispatch, select, bridge }) {
  const results = [];
...
export function normalizePlan(value, fallbackInput, doc) {
