...
export interface LogEntry { id: string; time: string; level: string; src: string; msg: string }
type Angles = Record<string, number>;

function push(arr: LogEntry[], entry: LogEntry) {
...
interface IntegrationState {
  ...
  servoOnlineCount: number;
  pendingAngles: Angles | null;
  lastSentAngles: Angles;
  simLog: LogEntry[];
  ctrlLog: LogEntry[];
  ...
  overcurrentServos: any[];
  setEspUrl: (url: string) => void;
  setConnected: (ok: boolean, latMs?: number | null) => void;
  setServoOnlineCount: (n: number) => void;
  setAvgVoltage: (v: number | null) => void;
  setTotalCurrentMA: (v: number | null) => void;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  setOvercurrentServos: (arr: any[]) => void;
  queueAngles: (angles: Angles) => boolean;
  consumeAngles: () => void;
  markSent: (angles: Angles) => void;
  simSent: (detail: string) => void;
  simFailed: (detail: string) => void;
  simOffline: (detail: string) => void;
  pushCtrlLog: (level: string, src: string, msg: string) => void;
  clearSimLog: () => void;
  clearCtrlLog: () => void;
  resetStats: () => void;
}
